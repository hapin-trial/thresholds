...
ecm_ingest <- function(file, tz="UTC", shiny=TRUE, output=c('raw_data', 'meta_data'), dummy='dummy_meta_data'){
  ...
  if(unprocessed=="?;m" || dim(fread(file, fill = TRUE))[1] < 30){
    ...
    if(date_formats[!is.na(value), length(value)]>1){
      ...
      revised_date_formats <- melt.data.table(data.table(
...
ecm_qa <- function(file, setShiny=TRUE){
  ...
  if(is.null(ingest)){return(NULL)}else{
    ...
    if('flags' %in% colnames(meta_data)){
      ...
      if(is.na(inletp_flag_condition)||is.null(inletp_flag_condition)||!is.numeric(inletp_flag_condition)){
        inletp_flag <- NA
      }else{
      ...
      flow_flag <- if(is.na(percent_flow_deviation)){NA}else if(percent_flow_deviation > flow_cutoff_threshold){1}else{0}
      ...
      meta_data[file %like% 'PERU|peru' & study_phase %like% "_07_", study_phase == '6']
