...
###For debugging only
# tz="UTC"
# lascar_files <- get_lascar_files_fun(); file<-lascar_files[1100]
    ...
    if(all(is.na(date_formats$value)) || badfileflag==1){
      ...
      # raw_data[, datetime:=as.POSIXct(datetime)]
	    ...
	    meta_data <- data.table(
				...
				datetime_instrument_on = min(raw_data$datetime),
				dur = dur
	    )
...
function(){
  lascar_path <- "~/Box/lascar_backup"
  lascar_files <- list.files(lascar_path, recursive=T, pattern='Las|las|LAS|USB|usb|Usb|CO|co|Co', full.names=T)
  lascar_files <- grep(".csv|.CSV|.txt|.TXT", lascar_files, value = TRUE)
  lascar_files <- grep(".xls", lascar_files, value = TRUE, invert = TRUE)
  lascar_files <- grep("ECM", lascar_files, value = TRUE, invert = TRUE)
  lascar_files <- grep("rds", lascar_files, value = TRUE, invert = TRUE)
}
